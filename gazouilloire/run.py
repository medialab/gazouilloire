...
def depiler(pile, db, debug=False):
    while True:
        ...
        for t in save:
             tid = db.save(t)
        if debug and save:
            sys.stderr.write("DEBUG: Saved %s tweets in MongoDB\n" % len(save))
        time.sleep(2)
...
if __name__=='__main__':
    try:
        ...
        StreamConn = TwitterStream(domain="stream.twitter.com", api_version="1.1", auth=oauth, secure=True, block=False, timeout=10, heartbeat_timeout=60)
