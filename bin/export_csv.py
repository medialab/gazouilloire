...
for t in db.find({}, sort=[("timestamp", -1)]):
    ...
    reply = "" if not t.get("in_reply_to_status_id_str", "") else "https://twitter.com/%s/status/%s" % (t.get("in_reply_to_screen_name", ""), t.get("in_reply_to_status_id_str", ""))
    print ",".join([a.encode("utf-8") for a in [url,name,ts,t.get("user_lang", ""),t.get("lang", ""),coords,text,reply,rt]])
